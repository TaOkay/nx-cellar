...
// ValidateSettings validates the provided AppSettings and returns an error if any
// field contains an invalid value.
func ValidateSettings(settings *AppSettings) error {
   if settings.ArchiveFolder != "" {
      if !filepath.IsAbs(settings.ArchiveFolder) {
         return errors.New("archive_folder must be an absolute path")
      }
      if len(settings.ArchiveFolder) > 260 {
         return errors.New("archive_folder must be 260 characters or fewer")
      }
   }

   if settings.GuiPagingSize < 1 || settings.GuiPagingSize > 500 {
      return errors.New("gui_page_size must be between 1 and 500")
   }
